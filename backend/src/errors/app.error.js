...
class OcCliNotFoundError extends AppError {
	constructor(
		details = "The OpenShift CLI (oc) is not installed or not available in PATH",
	) {
		super("OpenShift CLI is not available", {
			...
			action:
				"Install the OpenShift CLI and ensure the oc command is available in PATH.",
		});
	}
}
...
class KubernetesPermissionError extends AppError {
	constructor(
		details = "Your OpenShift user does not have permission for this resource",
	) {
		super("OpenShift permission denied", {
			...
			action:
				"Ask a cluster administrator for access or choose another project/pod.",
		});
	}
}
